...
// constructor
const TextEbook = function (ebook) {
...
TextEbook.findById = (ebookId, result) => {
    sql.query(`SELECT * FROM TextEbook WHERE Id = ${ebookId}`, (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(err, null);
            return;
        ...
        // not found TextEbook with the id
        result(null, null);
    });
};

TextEbook.findByKeyWord = (keyword, result) => {
    sql.query(`SELECT * FROM TextEbook WHERE Slug = '${keyword}'`, (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(err, null);
            return;
...
TextEbook.getAll = result => {
    sql.query("SELECT * FROM TextEbook", (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(null, err);
            return;
...
TextEbook.updateById = (id, ebook, result) => {
    sql.query(
        "UPDATE TextEbook SET Code = ?, Title = ?, Name = ?, Source = ?, ImageUrl = ?, OriginImageUrl = ?, View = ?, Slug = ?, Description = ?, Author = ?, CategoryString = ?, StatusString = ?, Rate = ?, OrtherName = ?, CrawlerDate = now() WHERE Id = ?",
        ...
        (err, res) => {
            ...
            // console.log("updated TextEbook: ", { id: id, ...ebook });
            result(null, { id: id, ...ebook });
        }
    );
};

TextEbook.updateImage = (id, imageUrl, result) => {
    sql.query(
        "UPDATE TextEbook SET OriginImageUrl = ?, CrawlerDate = now() WHERE Id = ?",
        [imageUrl, id],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
            ...
            console.log("updated TextEbook: " + id);
            result(null, { id: id, imageUrl:imageUrl });
        }
    );
};
module.exports = TextEbook;